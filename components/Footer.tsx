  ...
  return (
    <footer id="footer" className="flexCenter mb-24">
      ...
      flex-col gap-14">
              ...
              <FooterColumn title={columns.title}>
                <ul className="regular-14 flex flex-col gap-4 text-gray-30">
                  {columns.links.map((link) => (
                    <Link href='#footer' key={link}>{link}</Link>
                  ))}
                </ul>
              </FooterColumn>
            ))}
            ...
            <div className="flex flex-col gap-5">
              <FooterColumn title={FOOTER_CONTACT_INFO.title}>
                {FOOTER_CONTACT_INFO.links.map((link) => (
                  <Link href='#footer' key={link.label}
        ...
        <p className="regular-14 w-full text-center text-gray-30">2024 Guru BootCamp | All right reserved</p>
      </div>
    </footer>
  )
}
